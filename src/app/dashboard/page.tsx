    ...
    return (
      <section className="section-shell py-12 sm:py-16">
        <div className="glass-panel mx-auto max-w-3xl p-8 text-center">
          <p className="text-sm uppercase tracking-[0.3em] text-cyan-300">{t.profileCenter}</p>
    ...
    <section className="section-shell py-12 sm:py-16">
      ...
      <div className="mt-10 glass-panel p-6 sm:p-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <span className="flex h-18 w-18 items-center justify-center rounded-full border border-cyan-400/30 bg-cyan-400/15 text-3xl font-semibold text-cyan-200">
              {avatarInitial}
            </span>
            <div>
              <p className="text-2xl font-semibold text-white">{dashboard.profile.name}</p>
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.followers}</p>
            <p className="mt-2 text-xl font-semibold text-white">{dashboard.profile.followers}</p>
          </div>
          <div className="rounded-2xl bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.joinedAt}</p>
            <p className="mt-2 text-xl font-semibold text-white">{dashboard.profile.joinedAt}</p>
          </div>
          <div className="rounded-2xl bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.pendingReviews}</p>
            <p className="mt-2 text-xl font-semibold text-white">{dashboard.pendingCount}</p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-8 glass-panel p-8">
        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-white">{t.earningsTitle}</h2>
            <p className="mt-2 text-slate-300">{t.earningsDesc}</p>
          </div>
          <Link href="#support-center" className="rounded-full bg-cyan-400 px-5 py-3 text-sm font-semibold text-slate-950">
        ...
        <div className="mt-6 grid gap-4 md:grid-cols-3">
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.totalEarned}</p>
            <p className="mt-2 text-3xl font-semibold text-emerald-300">HK${dashboard.totalEarned.toLocaleString()}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.availableToWithdraw}</p>
            <p className="mt-2 text-3xl font-semibold text-white">HK${dashboard.availableToWithdraw.toLocaleString()}</p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
            <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.withdrawnSoFar}</p>
            <p className="mt-2 text-3xl font-semibold text-white">HK${dashboard.withdrawnSoFar.toLocaleString()}</p>
          </div>
        </div>

        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-sm text-slate-300">
          <p className="text-xs uppercase tracking-[0.16em] text-slate-400">{t.recentWithdrawal}</p>

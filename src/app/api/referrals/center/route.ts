...
export async function GET(request: Request) {
  ...
  if (!supabase || !admin) {
    return NextResponse.json({ error: isZh ? "推薦服務暫時不可用，請稍後再試。" : "Supabase is not configured." }, { status: 503 });
  }
  ...
  if (!user) {
    return NextResponse.json({ error: isZh ? "請先登入後再使用推薦功能。" : "Authentication required." }, { status: 401 });
  }

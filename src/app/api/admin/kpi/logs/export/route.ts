...
export async function GET(request: Request) {
  ...
  if (!supabase || !admin) {
    return NextResponse.json({ error: isZh ? "日誌匯出服務暫時不可用，請稍後再試。" : "Supabase admin mode is not configured." }, { status: 503 });
  }
  ...
  if (error) {
    return NextResponse.json({ error: isZh ? "匯出系統日誌失敗，請稍後再試。" : error.message }, { status: 400 });
  }
